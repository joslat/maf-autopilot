...
2. **Length caps on every user-controlled string input.** Call `BoundedInput.Validate(value, maxBytes, paramName)` at the top of the method (the helper lands in Phase 5.1 of the security-hardening release — until then, document the intended cap in a `// TODO(BoundedInput)` comment at each input, so the retrofit is mechanical). Caps by class: file path = 4 KB; identifier / package id / version = 256 B; short user text = 16 KB; code snippet = 256 KB; instructions = 64 KB; multi-file aggregate = 100 MB. Failing this is the most common source of DoS surface.
...
5. **LLM-bound user content is fenced.** Any string that ends up in another LLM's context (whether served as a tool output rendered by a downstream agent, or via an MCP Prompt template) must wrap user-controlled slots via `LlmFencing.Fence("label", value, maxBytes)` (helper lands in Phase 2.1 of the security-hardening release; until then, document the intent with a `// TODO(LlmFencing)` comment at each call site). This applies to tool descriptions, prompt templates, and tool outputs that are designed to be passed to a sibling MCP tool (e.g. `MafDraftIssue` → GitHub `create_issue`).
