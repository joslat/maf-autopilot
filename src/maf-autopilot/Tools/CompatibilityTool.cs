    ...
    internal static readonly IReadOnlyDictionary<string, string> Matrix =
        new SortedDictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["1.6.1"] = """
                ...
                Breaking: `WorkflowEvaluationExtensions.EvaluateAsync` adds `string? expectedOutput` before
                `CancellationToken`; positional call sites passing token positionally must be updated.
                Runtime behavior: `OpenTelemetryAgent` now auto-wires OpenTelemetry chat client instrumentation.
                """,
