<!--
Paste-able snippet for `.claude/maf-doctor.md`, a sidecar imported by a
one-line `@.claude/maf-doctor.md` in `CLAUDE.md` at the repo root — Claude
Code auto-loads `CLAUDE.md`, which pulls this in.

`maf-doctor init` writes the sidecar and OVERWRITES it on every re-run —
don't hand-edit the delivered copy. For durable, hand-authored guidance, edit
`CLAUDE.md` itself outside the managed `@import` line (init never touches it).
-->
