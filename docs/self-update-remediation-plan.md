...
### 2026-06-14 — Decision reversal: FULL package + CLI rename to `maf-doctor` (supersedes 6.x "identity frozen")
- **Context:** §6 and 6.3's "Rejected" note (`:357`) deliberately froze the technical identity (`PackageId`/`ToolCommandName`/`AssemblyName`/namespace = `maf-autopilot`) because a NuGet-id rename normally means republish + lifelong dual-maintenance and would break the `command=maf-autopilot` line `init` had written into users' `mcp.json`. **The maintainer overrode that call:** *"We have not announced this project, so we can break it."* No public installs exist yet, so the dual-maintenance/break-existing-users cost the rejection was built on doesn't apply.
- **Shipped (PR #81, squash `1a0b29e`, merged to `main`):** `<PackageId>` / `<ToolCommandName>` / `<AssemblyName>` → `maf-doctor`; analyzer → `maf-doctor.Analyzers`; both csprojs `1.2.0`; Dockerfile ENTRYPOINT + all doc/workflow command refs → `maf-doctor`. `InitCommand` now writes server key `maf-doctor` **and** `command: maf-doctor`, while still recognizing the legacy `maf-autopilot` key on re-`init`. New `docs/quickstart.md` (5-beat video walkthrough) linked from README. Build green; 739 + 15 tests pass; workflows parse; no stray `src/maf-doctor` paths.
- **Kept internal (small-diff):** `src/maf-autopilot/` dir, `.csproj`/`.sln` filenames, `*.Tests` assembly names, and the `MafAutopilot` C# namespace (6.5 still optional/separate).
- **Known consequence — accepted by maintainer:** scheduled `drift`/`pr-audit`/`verify` all run `dotnet tool install maf-doctor`, which 404s (`not found in NuGet feeds`) until the new id is published → those checks are RED on `main`. When asked (merge now vs hold), the maintainer chose **"merge now, accept red CI."** 🔒 **Maintainer TODO ("in some time"):** create a NuGet API key for the `maf-doctor` id, publish `1.2.0`, deprecate the old `maf-autopilot` package. This reopens what 6.3 had marked rejected — track it as the one remaining gate to a fully-green board.
