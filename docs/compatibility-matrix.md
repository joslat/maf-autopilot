...
|-------------|------------------------|------|-----------------|--------------------|-------|
| **1.15.0** | `≥ 10.6.0` | `≥ 8.0` | _(not pinned by MAF — BYO via IChatClient)_ | `1.15.0` | **Breaking** — review required. API summary: 10 breaking. See `guides/maf-1.15.0-migration-guide.md` and the registry entries. Transitive pins carried from 1.14.0 (verify). |
