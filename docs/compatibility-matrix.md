...
|-------------|------------------------|------|-----------------|--------------------|-------|
| **1.16.0** | `≥ 10.6.0` | `≥ 8.0` | _(not pinned by MAF — BYO via IChatClient)_ | `1.16.0` | Additive release — no `.NET … [BREAKING]` changes and no breaking or potentially-breaking API rows (source-compatible). New members: `WithResponseLanguage`, `WithPromptOverrides`. Transitive pins carried from 1.15.0. |
