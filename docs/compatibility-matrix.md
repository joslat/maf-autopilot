...
|-------------|------------------------|------|-----------------|--------------------|-------|
| **1.17.0** | `≥ 10.7.0` | `≥ 8.0` | _(not pinned by MAF — BYO via IChatClient)_ | `1.17.0` | Additive release — no `.NET … [BREAKING]` changes and no breaking or potentially-breaking API rows (source-compatible). Lifecycle transitions: Informational: repository externalization: microsoft/agent-framework → microsoft/agent-framework-durable-extension; package IDs unchanged. Transitive pins carried from 1.16.0. |
...
Current tracked version: **`1.17.0`** (see `.maf-version`)
