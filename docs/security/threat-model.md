...
- `ProcessRunner` (backing `dotnet build`, `dotnet-inspect diff`, and `git`) read stdout/stderr fully via `ReadToEndAsync` before truncating at a 16 MB cap — the cap bounded the *returned* string, not memory used *while reading*. **Mitigated:** streams both pipes concurrently against a shared byte budget, truncating during the read.
